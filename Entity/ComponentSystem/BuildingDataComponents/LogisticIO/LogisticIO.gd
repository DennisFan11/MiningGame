...
func _on_setuped():
	...
	assert(__building_controller, "no building controller")
	
	# 註冊物流組件
	_logistic_manager.set_logistic(__building_controller.coord, self)
	# 端口重新綁定
	_logistic_manager.port_rebind(__building_controller.coord)
	# 組件隨節點移除
	tree_exiting.connect(
		_logistic_manager.erase_logistic.bind(__building_controller.coord))


var __item_transport: IItemTransport
var _logistic_manager: LogisticManager
var __building_controller: BuildingController:
	set(new):
		__building_controller = new
		#_logistic_manager.port_rebind(__building_controller.coord)## 組件轉向
...
func _set_input(global_coord: Vector2i, item_source: IItemTransport):
	var key := Vector2(global_coord - __building_controller.coord).rotated(
		- global_rotation
	)
...
func port_rebind_ALG():
	...
	for in_coord: Vector2i in _get_input_coords():
		...
		if component:
			var item_source := component._has_output_targeting(__building_controller.coord)
			if item_source:
